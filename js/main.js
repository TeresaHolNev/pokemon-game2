...
//Creación de clase rejilla y divs de cada personaje a partir de array
...
function baraja() {
    ...
    btnInicio.style.display = "none";   //Quitar el boton mientras se juega
    reloj.style.display = "initial";
    cartelGameOver.style.opacity = "0";
    eliminados = 0;
    ganador.classList.remove("open");   //Cuando ganes se quita la imagen para volver a barajar
    song.currentTime = 0;
    song.play();
    song.volume = 0.5;
};
...
//Función para ejecutar la lógica de gameOver

function gameOver() {
    segundos = 150;
    rejilla.classList.add("fuera");         //Para que no aparezcan mas de 24 bolas al darle a start
    ...
    song.pause();
    setTimeout (function (){
        while(rejilla.firstChild) {         //Mientras rejilla tenga un hijo y sea verdad
            rejilla.removeChild(rejilla.firstChild);    //Quita ese primer hijo
        };
    },1000);
};
...
//Funcón para resetear los selccionados cuando no coincidan
